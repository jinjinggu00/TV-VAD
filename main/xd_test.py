...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    with torch.no_grad():
        ...
        for i, item in enumerate(testdataloader):
            # print(type(item), len(item), item[0].shape, item[1], item[2]) # <class 'list'> 3 torch.Size([1, 256, 512]) ('G-B2-B6',) tensor([67])
            ...
            tensor_list.append(lengths)




            padding_mask = get_batch_mask(lengths, maxlen).to(device)
            _, logits1, logits2  = model(visual, padding_mask, prompt_text, lengths) # logist1 shape is torch.Size([2, 256, 1]) logist2 shape is: torch.Size([2, 256, 7])
            # print('logist1 shape is', logits1.shape, 'logist2 shape is:', logits2.shape)
...
if __name__ == '__main__':
    ...
    gt = np.load(args.gt_path)  # len = 2331296
